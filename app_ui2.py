import streamlit as st
import pandas as pd
import numpy as np
import shap
import requests
import matplotlib.pyplot as plt
import plotly.express as px

# ========================
# Page config
# ========================
st.set_page_config(
    page_title="NovaPay Fraud Detection",
    layout="wide",
    page_icon="💸"
)

# ========================
# Sidebar navigation
# ========================
menu = st.sidebar.selectbox("Navigation", ["Home", "Detect Fraud", "Summary"])

# ========================
# Home page content
# ========================
if menu == "Home":
    st.title("🔍 NovaPay Fraud Detection System")
    st.markdown("""
        This application allows you to detect potentially fraudulent transactions using 
        a LightGBM model served via FastAPI.
        
        ➤ Navigate to **Detect Fraud** to input transaction data and receive a prediction.
        ➤ The model returns probability and explanation insights.
        
        **Built With:** FastAPI, Streamlit, LightGBM.
    """)

# ========================
# Detect Fraud page
# ========================
elif menu == "Detect Fraud":
    st.title("🛡️ Detect Fraudulent Transaction")

    with st.form("fraud_form", clear_on_submit=False):
        col1, col2 = st.columns(2)

        with col1:
            ip_risk_score = st.slider("IP Risk Score", 0.0, 10.0, 2.5)
            risk_score_internal = st.slider("Internal Risk Score", 0.0, 10.0, 2.5)
            account_age_days = st.number_input("Account Age (days)", 0, 5000, 365)
            fee = st.number_input("Transaction Fee", 0.0, 1000.0, 0.5)
            kyc_tier_enhanced = st.selectbox("KYC Tier Enhanced", [0, 1])
            txn_velocity_1h = st.slider("Transactions (1h)", 0.0, 20.0, 3.0)
            amount_src = st.number_input("Source Amount", 0.0, 10000.0, 500.0)
            device_trust_score = st.slider("Device Trust Score", 0.0, 10.0, 0.8)
            txn_velocity_24h = st.slider("Transactions (24h)", 0.0, 50.0, 4.3)
            amount_usd = st.number_input("USD Amount", 0.0, 10000.0, 520.0)

        with col2:
            exchange_rate_src_to_dest = st.number_input("Exchange Rate", 0.0, 5.0, 1.0)
            location_mismatch = st.selectbox("Location Mismatch", [0, 1])
            dest_currency_USD = st.selectbox("Destination Currency - USD", [0, 1])
            corridor_risk = st.slider("Corridor Risk", 0.0, 10.0, 0.3)
            channel_web = st.selectbox("Web Channel", [0, 1])
            channel_mobile = st.selectbox("Mobile Channel", [0, 1])
            dest_currency_CAD = st.selectbox("Destination Currency - CAD", [0, 1])
            ip_country_US = st.selectbox("IP Country - US", [0, 1])
            dest_currency_MXN = st.selectbox("Destination Currency - MXN", [0, 1])
            dest_currency_INR = st.selectbox("Destination Currency - INR", [0, 1])

        submitted = st.form_submit_button("🔍 Detect Fraud")

    # Payload for API
    payload = {
        "ip_risk_score": ip_risk_score,
        "risk_score_internal": risk_score_internal,
        "account_age_days": account_age_days,
        "fee": fee,
        "kyc_tier_enhanced": kyc_tier_enhanced,
        "txn_velocity_1h": txn_velocity_1h,
        "amount_src": amount_src,
        "device_trust_score": device_trust_score,
        "txn_velocity_24h": txn_velocity_24h,
        "amount_usd": amount_usd,
        "exchange_rate_src_to_dest": exchange_rate_src_to_dest,
        "location_mismatch": location_mismatch,
        "dest_currency_USD": dest_currency_USD,
        "corridor_risk": corridor_risk,
        "channel_web": channel_web,
        "channel_mobile": channel_mobile,
        "dest_currency_CAD": dest_currency_CAD,
        "ip_country_US": ip_country_US,
        "dest_currency_MXN": dest_currency_MXN,
        "dest_currency_INR": dest_currency_INR
    }

    if submitted:
        try:
            # -----------------------------
            # Call the backend API
            # -----------------------------
            response = requests.post("http://127.0.0.1:8000/predict/", json=payload)

            if response.status_code == 200:
                result = response.json()
                
                # Extract prediction
                pred = result.get("prediction")
                prob = result.get("fraud_probability")

                if pred is None:
                    st.error("API returned unexpected result. Check backend logs.")
                else:
                    label = "🚨 Fraudulent" if pred == 1 else "✔ Legitimate"
                    st.markdown(f"### **Result:** {label}")
                    st.markdown(f"#### Fraud Probability: {prob*100:.2f}%")

                    # Show optional explanation if provided
                    if "shap_values" in result:
                        st.subheader("🔍 Feature Impact (SHAP)")
                        shap_vals = np.array(result["shap_values"])
                        feat_names = list(payload.keys())
                        shap_df = pd.DataFrame({
                            "feature": feat_names,
                            "shap_value": shap_vals
                        })
                        shap_df["abs_val"] = shap_df["shap_value"].abs()
                        shap_df = shap_df.sort_values("abs_val", ascending=False)

                        fig, ax = plt.subplots(figsize=(8,6))
                        ax.barh(shap_df["feature"], shap_df["shap_value"], color='teal')
                        ax.set_title("SHAP Feature Contribution")
                        ax.set_ylabel("Feature")
                        ax.set_xlabel("SHAP Value")
                        st.pyplot(fig)

            else:
                st.error(f"API responded with status {response.status_code}: {response.text}")

        except Exception as e:
            st.error(f"Request failed: {e}")

# ========================
# Summary page
# ========================
elif menu == "Summary":
    st.title("📋 Project Summary & Recommendations")
    st.markdown("""
    ### Summary of the Fraud Detection Project

    - Built a real-time fraud detection system using Machine Learning.
    - Backend is powered by **FastAPI** and serves predictions via an API.
    - Frontend is powered by **Streamlit** and provides an interactive dashboard.
    - The model predicts fraud probability and can explain decisions via SHAP.

    ### Recommendations

    - Retrain model periodically with fresh data.
    - Log predictions for audit and monitoring.
    - Deploy backend behind a production server (e.g., Gunicorn + NGINX)
    - Deploy frontend on Streamlit Cloud or Render / Railway.

    **Thank You**
    """
    )
